...
int main() {
  ...
  ifstream ifs{"stations.txt"};
  char ch;
  char line[512] = "Bond Street";


    cout << line << endl;
    strcpy(route, "S,S,SE,S,N,NW,N,N,E,E,E,E,E,E,E,E,W,W,W,W,W,W,W,W");
    cout << "Starting at " << line << " and taking the steps:" << endl;
    cout << route << endl;
    result = validate_route(map, height, width, line, route, destination);
    if (result >= 0)
      cout << "is a valid route with " << result << " line change(s) ending at " << destination << "." << endl;
    else 
      cout << "is an invalid route (" << error_description(result) << ")" << endl;
  ...
  /* Deallocate all memory acquired by map */
  deallocate_2D_array(map, height);
